...
async function loadUserProfile() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error('불러오기 실패', response.status);
      alert('불러올 수 없습니다');
    }
  } catch (error) {
    console.error(error);
...
function updateProfileCard(userData) {
  ...
  const subscriptionDateElement = document.querySelector('#join-date');

  if (userData.username) {
    userNameElement.textContent = userData.username;
  }
  if (userData.email) {
    emailAddressElement.textContent = userData.email;
  }
  if (userData.subscriptionDate) {
    const date = new Date(userData.subscriptionDate);
    const year = date.getFullYear().toString().slice(-2);
    const month = ('0' + (date.getMonth() + 1)).slice(-2);
    const day = ('0' + date.getDate()).slice(-2);
    subscriptionDateElement.textContent = `${year}.${month}.${day}`;
  }
}
